...
bool File::isOpen(void) { return this->m_fd > 0; }
...
File::Status File::read(void* buffer, NATIVE_INT_TYPE& size, bool waitForFull) {
    ...
    // Validate read size before entering reading loop. Linux's read call
    // expects size_t, which is defined as an unsigned value. Thus 0 and
    ...
    NATIVE_INT_TYPE maxIters =
        size * 2;  // loop limit; couldn't block more times than number of bytes
...
File::Status File::write(const void* buffer, NATIVE_INT_TYPE& size,
    ...
    NATIVE_INT_TYPE maxIters =
        size * 2;  // loop limit; couldn't block more times than number of bytes
    while (maxIters > 0) {
        NATIVE_INT_TYPE writeSize = ::write(
            this->m_fd,
#ifdef __VXWORKS__
            static_cast<char*>(const_cast<void*>(
                buffer))  // Ugly, but that's how VxWorks likes to roll...
#else
            buffer
#endif
            ,
        ...
        if (-1 == writeSize) {
            switch (errno) {
                ...
                default:
                    DEBUG_PRINT(
                        "Error %d during write of 0x0%llx, addrMod %d, size "
                        "%d, sizeMod %d\n",
                        errno, (U64)buffer, ((U64)buffer) % 512, size,
                        size % 512);
                    stat = OTHER_ERROR;
                    break;
            }
...
// NOTE(mereweth) - see
// http://lkml.iu.edu/hypermail/linux/kernel/1005.2/01845.html recommendation
// from Linus Torvalds, but doesn't seem to be that fast
File::Status File::bulkWrite(const void* buffer, NATIVE_UINT_TYPE& totalSize,
    ...
    for (NATIVE_UINT_TYPE idx = 0; idx < totalSize; idx += chunkSize) {
        ...
        // if we're on the last iteration and length isn't a multiple of
        // chunkSize
        ...
        const Os::File::Status fileStatus =
            this->write((U8*)buffer + idx, size, false);
...
#ifdef __linux__
        sync_file_range(this->m_fd, startPosition + newBytesWritten, chunkSize,
                        SYNC_FILE_RANGE_WRITE);

        if (newBytesWritten) {
            sync_file_range(
                this->m_fd, startPosition + newBytesWritten - chunkSize,
                chunkSize,
...
NATIVE_INT_TYPE File::getLastError(void) { return this->m_lastError; }
